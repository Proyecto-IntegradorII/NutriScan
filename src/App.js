...
function App() {
	...
	async function getScoreFromServer() {
		...
		try {
			const response = await fetch(
				`https://speech-recognition-trivia-backend.vercel.app/score/${userId}`,
				{
					method: "GET",
					headers: {
	...
	async function updateScoreOnServer(newScore) {
		...
		try {
			const response = await fetch(
				"https://speech-recognition-trivia-backend.vercel.app/updatescore",
				{
					method: "POST",
					headers: {
						"Content-Type": "application/json",
	...
	return (
		<section className="container bg-white mx-auto p-5 mt-12 xl:px-64 fixed inset-0">
			<div className="items-center  bg-white w-full h-full flex flex-col">
					...
					{machineQuestionsAndOptions && (
						<div>
							<div className="rounded-lg bg-blue-200 p-4">
								<div
									dangerouslySetInnerHTML={{ __html: machineQuestionsAndOptions["question"] }}
								></div>
							</div>
							<div className="grid grid-cols-2 gap-4 mt-4">
								<div className="rounded-lg bg-blue-200 p-4 mt-0 w-5/5 ">
									<div
										ref={optionADivRef}
										dangerouslySetInnerHTML={{ __html: machineQuestionsAndOptions["optionA"] }}
									></div>
								</div>
								<div className="rounded-lg bg-blue-200 p-4 mt-0 w-5/5 ">
									<div
										ref={optionBDivRef}
										dangerouslySetInnerHTML={{ __html: machineQuestionsAndOptions["optionB"] }}
									></div>
								</div>
								<div className="rounded-lg bg-blue-200 p-4 mt-0 w-5/5 ">
									<div
										ref={optionCDivRef}
										dangerouslySetInnerHTML={{ __html: machineQuestionsAndOptions["optionC"] }}
									></div>
								</div>
								<div className="rounded-lg bg-blue-200 p-4 mt-0 w-5/5 ">
									<div
										ref={optionDDivRef}
										dangerouslySetInnerHTML={{ __html: machineQuestionsAndOptions["optionD"] }}
									></div>
								</div>
							</div>
						</div>
									...
									>
										<div className="chat-image avatar">
											<div className="w-10 rounded-full">
												<img
													src={
														msg.role === "assistant" ? "/images/logo.png " : "/images/person.png"
													}
												/>
											</div>
										</div>
				...
				<form
					className="form-control xl:w-[800px] md:w-full sm:w-full m-5 items-center bg-white"
