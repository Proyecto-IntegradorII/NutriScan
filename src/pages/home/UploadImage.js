...
const SendImage = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post('https://computervisionwithopencvbackend.onrender.com/get_characters_of_image', formData, {
        headers: {
  ...
  return (
    <div className="flex justify-center items-center h-screen pb-8">
      <form onSubmit={handleSubmit} className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 w-[36rem]">
        <div className="mb-4">
          <label htmlFor="image" className="block text-gray-700 text-xl font-bold mb-2">Subir la imagen de la tabla nutricional del producto que deseas consumir:</label>
          <input type="file" id="image" name="image" onChange={handleImageChange} className="border border-gray-400 rounded w-full py-16 px-16 text-gray-700 leading-tight focus:outline-none focus:border-blue-500" />
        </div>
        <div className="flex items-center justify-center">
          {isLoading ? (
            ...
            <button type="submit" className="w-48 z-10 bg-lime-900 hover:bg-lime-700 active:bg-lime-600 text-white font-bold py-2 px-4 rounded-xl text-xl border h-12 shadow-sm">Enviar</button>
          )}
        </div>
      </form>
    </div>
